...
<button><a href="pertemuan6/form.php">Tambah Data</a></button>
<table border="1">
	<tr>
		<th>NO</th>
		<th>NIM</th>
	...
	<tr>
		<td><?= $no++;?></td>
		<td>
			<a href="pertemuan4/latihan2.php?nim=<?= $row["nim"]; ?>&nama=<?= $row["nama"]; ?>&jurusan=<?= $row["jurusan"]; ?>&email=<?= $row["email"]; ?>&gambar=<?= $row["gambar"]; ?>">
				<?= $row["nim"];?>
			</a>
		</td>
		<td><?= $row["nama"];?></td>
		<td><?= $row["jurusan"];?></td>
		<td><?= $row["email"];?></td>
		<td> <a href="pertemuan6/ubah.php?id=<?= $row["id"]; ?>">Edit</a>| <a href="pertemuan6/hapus.php?id=<?= $row["id"]; ?>">Hapus</a></td>
	</tr>
<?php } ?>
</body>
</html>